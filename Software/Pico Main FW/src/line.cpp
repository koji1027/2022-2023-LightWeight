...
void Line::begin() {  // ラインセンサの初期化
    // ２個アナログマルチプレクサ(CD74HC4067)を使うためのピン設定
    ...
    for (uint8_t i = 0; i < SENSOR_NUM; i++) {  // しきい値の初期化
        threshold[i] = DEFAULT_THRESHOLD;
    }
}

void Line::read() {
    pre_line_theta = line_theta;  // 前回のラインの角度を保存
    line_theta = 0.0;             // ラインの角度を初期化
    // 各ラインセンサの値の読み取り
    for (int i = 0; i < 16; i++) {
        for (int j = 0; j < 4; j++) {
    ...
    // 白線の角度を計算する．（アルゴリズムについては"http://yuta.techblog.jp/archives/24145402.html"を参照）(多分これを使った)
...
void Line::set_threshold() {  // しきい値の自動設定
    int maxValue[32];
    int minValue[32];
    for (int i = 0; i < 32; i++) {
        maxValue[i] = 0;
