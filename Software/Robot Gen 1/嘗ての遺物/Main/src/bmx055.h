...
private:
    ...
    float pitch = 0.00;
    float yaw = 0.00;
    float roll_error = 0.00;
    float pitch_error = 0.00;
    float yaw_error = 0.00;
    float new_roll = 0.00;
    float new_pitch = 0.00;
    float new_yaw = 0.00;
    float xaccl = 0.00;
    float yaccl = 0.00;
    float zaccl = 0.00;
...
void Axis::init()
{
    ...
    float _roll_error = 0.00;
    float _pitch_error = 0.00;
    float _yaw_error = 0.00;
    for (int i = 0; i < 20; i++)
    {
        get_rad();
        _roll_error -= roll;
    ...
    roll_error = _roll_error;
    pitch_error = _pitch_error;
...
void Axis::get_rad()
{
    ...
    roll = atan2(yaccl, sqrt(xaccl * xaccl + zaccl * zaccl));
    pitch = atan2(xaccl, sqrt(yaccl * yaccl + zaccl * zaccl));
    float numerator = cos(roll) * ymag - sin(roll) * zmag;
    float denominator = cos(pitch) * xmag + sin(pitch) * sin(roll) * ymag + sin(pitch) * cos(roll) * zmag;
    yaw = -atan2(numerator, denominator);
    xgyro *= PI / 180.0;
    ygyro *= PI / 180.0;
    zgyro *= PI / 180.0;

    new_roll = k * (new_roll + xgyro * dt) + (1 - k) * roll;
    new_pitch = k * (new_pitch + ygyro * dt) + (1 - k) * pitch;
    new_yaw = k * (new_yaw + zgyro * dt) + (1 - k) * yaw;

    roll = degrees(new_roll);
    pitch = degrees(new_pitch);
    yaw = degrees(new_yaw);
