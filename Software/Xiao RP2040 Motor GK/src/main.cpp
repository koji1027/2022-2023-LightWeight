...
void uart_recv(void) {
    ...
    if (header == 255)                // ヘッダーが正しいときデータを受け取る
    {
        ...
        if (Serial1.read() == 254)  // フッターが正しい時データを解析
        {
            if (buf[0] != 255 && buf[1] != 255 && buf[2] != 255 && buf[3] != 255 && buf[4] != 255 && buf[5] != 255 && buf[6] != 255 && buf[7] != 255)  // どのデータもヘッダー（255）と被らないときデータを利用
            {
