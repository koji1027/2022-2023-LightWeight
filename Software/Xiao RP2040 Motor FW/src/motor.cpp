...
void Motor::cal(double move_angle, double gyro_angle, double machine_angle, uint8_t speed)
{
        ...
        double gyro_angle_diff = machine_angle - gyro_angle; // ジャイロの角度と機体の向いているべき方向の差
        unsigned long long now_time = micros(); // 現在の時間
        unsigned long long dt = now_time - pre_time; // 前回の処理を行った時の時間からの経過時間
        ...
        if (abs(gyro_angle_diff) < PI / 4.0)
        {
                ...
                for (uint8_t i = 0; i < MOTOR_NUM; i++) 
                {
                        power_ratio[i] = sin(move_angle - MOTOR_RAD[i]); // モーターの出力比を計算（計算法については，別途説明）
                }
                double max = 0; //最大の出力比
                for (uint8_t i = 0; i < MOTOR_NUM; i++)
                {
        ...
        for (uint8_t i = 0; i < MOTOR_NUM; i++)
        {
                power[i] = pre_p[i] * POWER_LPF + power[i] * (1 - POWER_LPF); // 出力にLPFをかける
