  ...
  if (header == 255) { // ヘッダーが正しいときデータを受け取る
    ...
    if (Serial1.read() == 254) { // フッターが正しいときデータを解析
      if (buf[0] != 255 && buf[1] != 255 && buf[2] != 255 && buf[3] != 255 &&
          buf[4] != 255 && buf[5] != 255 && buf[6] != 255 &&
          buf[7] != 255)  // どのデータもヘッダー（255）と被らないときデータを利用
      {
        flag = buf[0]; // モーターの状態に関するフラグを受け取る
        move_angle =
            (buf[1] + (buf[2] << 7)) / 100.0 - PI;  //移動方向 0 ~ 200 -> -PI ~ PI
        gyro_angle =
            (buf[3] + (buf[4] << 7)) / 100.0 - PI;  //ジャイロセンサで得られる機体の向き 0 ~ 200 -> -PI ~ PI
        machine_angle =
            (buf[5] + (buf[6] << 7)) / 100.0 - PI;  //機体が向くべき方向 0 ~ 200 -> -PI ~ PI
